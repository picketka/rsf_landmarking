#anything from above repeated # Simulation settings  ------------------------------------------------------------------------
...
BS <- function(timepoints,times,status,pred,cause=1){ 
  ...
  for(t in 1:n_times){
    ...
    BS[t] <- (sum(hit1_all[,t], na.rm = T) +sum(hit2_all[,t], na.rm = T))/n
  } 
...
## 1. Simulate baseline covariates (how many? Continuous? Binary?) 
  ...
  ############################################JM Prediction + Cox and RSF Models#############################
  for(t0 in LMx){
